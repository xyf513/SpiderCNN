...
parser.add_argument('--model', default='spidercnn_cls', help='Model name: spidercnn [default: spidercnn]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            # Get model and loss 
            pred = MODEL.get_model(pointclouds_pl, is_training_pl, bn_decay=bn_decay, num_classes=NUM_CLASSES)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for fn in range(len(TEST_FILES)):
        log_string('----' + str(fn) + '-----')
        current_data, current_label, normal_data = provider.loadDataFile_with_normal(TEST_FILES[fn])
        normal_data = normal_data[:,0:NUM_POINT,:]
        current_data = current_data[:,0:NUM_POINT,:]
        current_label = np.squeeze(current_label)
